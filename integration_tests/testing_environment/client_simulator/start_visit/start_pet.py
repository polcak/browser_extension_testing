...
def test_pet(exp, os, config, browser, pet, repeats, exp_config, server_config):
    ...
    try:
        ...
        repeats = int(repeats) if not isinstance(repeats, (int, int)) else repeats

        print("configure the browser")
        unit = sim_browser(config, browser, pet, proxy_setting)
        time.sleep(5)
        print("visit sites")
...
def sim_browser(config, browser, pet, proxy_setting):
    b = Browser(config, browser, pet, proxy_setting)
    return b

def cross_session(unit, exp, os, config, browser, pet, fp_site):
    html_args = "?exp="+exp+"&os="+os+"&config="+config+"&browser="+browser+"&pet="+'_'.join(pet)+"&type=session"
    fp_site += html_args
    unit.visit_sites([fp_site+"&sess=0"])
    print("Sleeping since", time.strftime("%a, %d %b %Y %H:%M:%S +0000", localtime()))
    time.sleep(2700)        # for 2700 seconds == 45 minutes
    unit.visit_sites([fp_site+"&sess=1"])
    
def cross_site(unit, exp, os, config, browser, pet,  fp_sites):
    time.sleep(10)
    for fp_site in fp_sites:
        html_args = "?exp="+exp+"&os="+os+"&config="+config+"&browser="+browser+"&pet="+'_'.join(pet)
        fp_site += html_args
        unit.visit_sites([fp_site])

def cross_reload(unit, exp, os, config, browser, pet, repeats, fp_site):
    html_args = "?exp="+exp+"&os="+os+"&config="+config+"&browser="+browser+"&pet="+'_'.join(pet)+"&type=reload"
    fp_site += html_args
    repeats = int(repeats)
...
if __name__ == '__main__':
    if(len(sys.argv) != 9):
        print("Call as follows: python start_pet.py <exp> <os> <config> <browser> <pet> <repeats> <exp_config> <server_config>")
        sys.exit(0)
    ...
    PET = sys.argv[5]
    REPEATS = sys.argv[6]
    EXP_CONFIG = sys.argv[7] # "vm_all.json" etc.
    SERVER_CONFIG = sys.argv[8] # "vm_server.json" etc
